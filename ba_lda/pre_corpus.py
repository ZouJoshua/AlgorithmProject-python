...
def load_json_file(filename):
    ...
    f = open(filename, 'r')
    for line in f:
        doc = re.findall(r'\w+(?:\'\w+)?', json.loads(line.strip())['article_title'])
        if len(doc) > 0:
            corpus.append(doc)
    f.close()
    return corpus
...
if __name__ == "__main__":
    corpus = load_corpus("0:500")
    with open("/home/joshua/PycharmProjects/AlgorithmProject-python/data/lda/train", "w") as f:
        for i in corpus:
            f.write(" ".join(i))
            f.write("\n")
    print(corpus)
