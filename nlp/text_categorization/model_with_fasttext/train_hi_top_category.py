...
class TopCategoryModel(object):
    ...
    def __init__(self, dataDir, category='top', k=5, model_level='one_level'):
        ...
        self.label_idx_map, self.idx_label_map = self._get_label(os.path.join(root_nlp_path, 'data', 'n_hi_category_data', 'label_idx_map.json'))
    ...
    def _preline(self, line):
        ...
        content = ""
        dataY = str(self.label_idx_map[line_json["top_category"].strip().lower()])
        if "content" in line_json.keys():
            content = line_json["content"]
        elif "html" in line_json.keys():
    ...
    def train_model(self):
        ...
        for i in range(self.k):
            ...
            if os.path.exists(data_path):
                ...
                self.evaluate_model(test_check_pred_path, model_level=self._level, model_num=_model)
            else:
                pass
...
if __name__ == '__main__':
    s = time.time()
    dataDir = "/data/in_hi_news/train_data"
