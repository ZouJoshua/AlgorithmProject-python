...
class Malayalam:
    ...
    def load_stop_words(self):
        stop_word_list = []
        f_obj = open('topic_modeling/stopwords/malayalam_stop_words.txt', 'r')
        stop_words = f_obj.readlines()

        for word in stop_words:
            malayalam_word, _ = word.split('    ')
            stop_word = malayalam_word.strip()
    ...
    def tokenize(self, text):
        ...
        malayalam_stop_words = self.load_stop_words()

        for word in tokenize_text:

            # steming
            w = Word(word, language="ml")
